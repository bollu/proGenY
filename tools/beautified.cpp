...
} //eventProcess::_handleWindowCloseEvent
...
} //eventProcess::_handleMouseButtonPressed
...
} //eventProcess::_handleMouseButtonReleased
...
} //eventProcess::_handleKeyboardPressed
...
} //eventProcess::_handleKeyboardReleased
...
} //eventProcess::_handleMouseWheelMove
...
public:
	...
	vector2 clamp ( vector2 minVec, vector2 maxVec ){
		...
		if ( clampedVec.x < minVec.x ) {
			clampedVec.x = minVec.x;
		}

		if ( clampedVec.x > maxVec.x ) {
			clampedVec.x = maxVec.x;
		}

		if ( clampedVec.y < minVec.y ) {
			clampedVec.y = minVec.y;
		}

		if ( clampedVec.y > maxVec.y ) {
			clampedVec.y = maxVec.y;
		}
	...
	inline vector2 operator + ( const vector2 & a ) const { return ( vector2( x + a.x, y +
		                                                                  a.y ) ); }
	...
	inline vector2 operator * ( float scale )          const { return ( vector2( x * scale, y *
		                                                                     scale ) ); }
...
inline vector2 operator * ( const TYPE s,
		const vector2	     & a ){ return ( vector2( a.x * s, a.y * s ) );    }

template< typename TYPE >

inline vector2 operator * ( const vector2 & a,
		const TYPE		    s ){ return ( vector2( a.x * s, a.y * s ) );    }
...
public:
	...
	inline vector3 operator + ( const vector3 & a ){ return ( vector3( x + a.x, y + a.y, z +
		                                                           a.z ) ); }
	...
	//template<class otherPhyVect> inline operator otherPhyVect(){ return otherPhyVect(this->x,
	//this->y, this->z); }
	template< class otherPhyVect2 >
	inline operator otherPhyVect2 (){ return ( otherPhyVect2( this->x, this->y ) ); }
...
inline vector3 operator * ( float s,
		const vector3	& a ){ return ( vector3( a.x * s, a.y * s, a.z * s ) );    }

inline vector3 operator * ( const vector3 & a,
		float			    s ){ return ( vector3( a.x * s, a.y * s, a.z * s ) );
}

inline bool operator == ( const vector3 &a,
		vector3		       & b ){ return ( (a.x == a.y) && (b.x == b.y) &&
	                                               (a.z == b.z) );  }
...
public:
	...
	void addObject ( Object *obj ){
		...
		for ( objProcessorIt it = this->objProcessors.begin() ;
		      it != this->objProcessors.end() ;
		      ++it ) {
			(*it)->onObjectAdd( obj );
		}
	} //addObject
...
void gameState::_Init (){
	objectMgrProcess *objMgrProc = this->processManager->getProcess< objectMgrProcess > ( Hash::getHash(
	                                                                                              "objectMgrProcess" ) );


	this->objectManager = objMgrProc->getObjectMgr();
	this->viewProc	    =
	        this->processManager->getProcess< viewProcess > ( Hash::getHash( "viewProcess" ) );
	this->_initFactory();
...
void gameState::_initFactory (){
	...
	this->objFactory.attachObjectCreator( Hash::getHash( "gun" ), new gunCreator(
	                                              this->viewProc ) );
...
} //gameState::_initFactory


void gameState::_generateTerrain ( unsigned long long seed, vector2 playerInitPos,
		vector2 levelDim ){
	terrainCreator *creator = objFactory.getCreator< terrainCreator > ( Hash::getHash(
	                                                                            "terrain" ) );
	vector2 blockDim	=  vector2( 64, 64 );
	vector2 terrainDim	=  vector2( levelDim.x / blockDim.x, levelDim.y / blockDim.y );
	...
	creator->reserveRectSpace( playerInitPos, vector2( 256, 256 ) * render2GameCoord );

	Object *terrainObj = creator->createObject();
} //gameState::_generateTerrain


void gameState::_createPlayer ( vector2 playerInitPos, vector2 levelDim ){
	playerCreator *creator =
	        objFactory.getCreator< playerCreator > ( Hash::getHash( "player" ) );
	...
	this->_playerController = new playerController( this->eventManager,
	                                                this->objectManager,
	                                                &this->objFactory,
	                                                this->viewProc );
	this->_playerController->createPlayer( levelDim, playerInitPos, creator, playerData );
	{
		bladeData blade;
		bladeCreator *creator =
		        this->objFactory.getCreator< bladeCreator > ( Hash::getHash( "blade" ) );
...
void gameState::_createDummy ( vector2 levelDim ){
	{
		dummyCreator *creator =
		        objFactory.getCreator< dummyCreator > ( Hash::getHash( "dummy" ) );
	...
	}
	{
		pickupCreator *creator =
		        objFactory.getCreator< pickupCreator > ( Hash::getHash( "pickup" ) );
		...
		data.eventData	   =
		        new Prop< gunDataGenerator > ( gunDataGenerator( gunDataGenerator::Archetype
		                                                         ::Rocket,
		                                                         1, 10 ) );
...
Object * gameState::_createGuns ( Object *player, vector2 levelDim ){
	bulletCreator *_bulletCreator =
	        objFactory.getCreator< bulletCreator > ( Hash::getHash( "bullet" ) );
	gunCreator *creator =
	        objFactory.getCreator< gunCreator > ( Hash::getHash( "gun" ) );
	bulletData bullet;
	...
	return (gun);
} //gameState::_createGuns


void gameState::_generateBoundary ( vector2 levelDim ){
	boundaryCreator *creator =
	        objFactory.getCreator< boundaryCreator > ( Hash::getHash( "boundary" ) );
...
#endif //ifndef LOG_GLOBAL_OBJECTS
...
public:
	 ...
	 */
	scopedLog ( std::string onCreateMsg, std::string onDestroyMsg, logLevel level =
			        logLevelInfo ){
...
} //objContactListener::_extractPhyData
...
collisionData objContactListener::_fillCollisionData ( b2Contact *contact,
	...
	return (collision);
} //objContactListener::_fillCollisionData
...
} //objContactListener::_handleCollision