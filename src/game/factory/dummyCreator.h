...
public:
	dummyCreator ( viewProcess *_viewProc ) : viewProc( _viewProc ),
		                                  radius( 0 ){}
	...
	Object *createObject ( vector2 dummyPos ) const {
		...
		*pos = dummyPos;
		...
		dummyShape->SetAsBox( this->radius, this->radius );
		...
		//renderer------------------------------------
		sf::Shape *shape = renderUtil::createShape( dummyShape, viewProc );
		...
		dummy->addProp( Hash::getHash( "phyProp" ), phy);
		dummy->addProp( Hash::getHash( "healthData" ), new Prop< healthData > ( health ) );
