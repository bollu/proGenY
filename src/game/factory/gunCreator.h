...
public:
	gunCreator ( viewProcess *_viewProc ) : viewProc( _viewProc ),
		                                parent( NULL ),
		                                radius( 0 ){}
	...
	Object *createObject ( vector2 gunPos ) const {
		...
		*pos = gunPos;
		...
		//final---------------------------------
		gun->addProp( Hash::getHash( "renderData" ), new Prop< renderData > ( render ) );
		gun->addProp( Hash::getHash( "gunData" ), new Prop< gunData > ( this->gun ) );
		gun->addProp( Hash::getHash( "offsetData" ), new Prop< offsetData > ( offset ) );
