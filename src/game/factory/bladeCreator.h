...
public:
	bladeCreator ( viewProcess *_viewProc ) : viewProc( _viewProc ),
		                                  parent( NULL ),
		                                  radius( 0 ){}
	...
	Object *createObject ( vector2 pos ) const {
		...
		//final---------------------------------
		blade->addProp( Hash::getHash( "renderData" ), new Prop< renderData > ( render ) );
		blade->addProp( Hash::getHash( "bladeData" ),
		                new Prop< bladeData > ( this->blade ) );
		blade->addProp( Hash::getHash( "offsetData" ), new Prop< offsetData > ( offset ) );
