...
public:
	...
	Object *createObject ( vector2 playerInitPos ) const {
		...
		vector2 *pos	       =
		        boundaryObject->getProp< vector2 > ( Hash::getHash( "position" ) );


		*pos = vector2( 0, 0 ); //levelDim * 0.5; //+ vector2(thickness, thickness);

		phyProp *physicsData = new phyProp();
		renderData render;
		...
		physicsData->collisionType = Hash::getHash( "terrain" );
		{
			...
			vector2 bottomCenter   = vector2( levelDim.x / 2.0, 0 ); //vector2(levelDim.x
			                                                         /// 2.0, 0);
			...
			sf::Shape *shape = renderUtil::createShape( bottom, viewProc );

			shape->setFillColor( sf::Color::Blue );

			shapeRenderNode * renderer = new shapeRenderNode( shape,
		...
		}
		{
			...
			vector2 topCenter   = vector2( levelDim.x / 2.0, levelDim.y ); //vector2(levelDim.x
			                                                               /// 2.0,
			                                                               //levelDim.y
			                                                               /// 2.0);
			...
			sf::Shape *shape = renderUtil::createShape( top, viewProc );
		...
		}
		{
			...
			vector2 leftCenter   = vector2( 0, levelDim.y / 2.0 ); //vector2(0,
			                                                       //levelDim.y / 2.0);
			...
			sf::Shape *shape = renderUtil::createShape( left, viewProc );
		...
		}
		{
			...
			sf::Shape *shape = renderUtil::createShape( right, viewProc );
		...
		boundaryObject->addProp( Hash::getHash( "phyProp"), physicsData);
