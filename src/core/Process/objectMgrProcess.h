...
/*!Process to handle objectMgr
   ...
   Acts as a wrapper around objectMgr. this ensures that objectMgr is in sync with the other parts
      of
   the Engine.
...
public:
	objectMgrProcess ( processMgr &processManager, Settings &settings,
			eventMgr &eventManager ) : Process( "objectMgrProcess" ){
