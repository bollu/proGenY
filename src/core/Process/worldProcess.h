...
private:
	...
	//if the amount accumilated passes maxAccumilation, the accumilator "lapses". this is to
	//make sure
	//that it dosen't get sucked into a "spiral of doom" where it updates more as it's
	//accumilated a lot,
...
public:
	worldProcess ( processMgr &processManager, Settings &settings,
			eventMgr &eventManager ) : Process( "worldProcess" ){
		...
		this->stepSize		  =
		        *settings.getProp< float > ( Hash::getHash( "stepSize" ) );
		this->velIterations	  =
		        *settings.getProp< int > ( Hash::getHash( "velIterations" ) );
		this->collisionIterations =
		        *settings.getProp< int > ( Hash::getHash( "collisionIterations" ) );
	...
	void Update ( float dt ){
		...
		while ( dtAccumilator >= stepSize ) {
			...
			//now decrement the accumilator by the step size. repeat until you have less
			//accumilated
			//than a step size
			dtAccumilator -= stepSize;
		}
