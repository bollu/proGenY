...
public:
	windowProcess ( processMgr &processManager, Settings &settings,
			eventMgr &eventManager ) : Process( "windowProcess" ){
		vector2 *screenDim = settings.getProp< vector2 > ( Hash::getHash(
		                                                           "screenDimensions" ) );
		...
		this->window		  =
		        new sf::RenderWindow( sf::VideoMode( screenDim->x,
		                                             screenDim->y ), "ProGenY", sf::Style::Default,
		                              context );
